-- Adicione ao seu arquivo de plugins (~/.config/nvim/lua/plugins/ui.lua)
return {
  {
    "goolord/alpha-nvim",
    event = "VimEnter",
    config = function()
      local alpha = require("alpha")
      local dashboard = require("alpha.themes.dashboard")
      
      -- Seu ASCII personalizado (exemplo minimalista)
      dashboard.section.header.val = {
        "                        .'.     ",
        "                       /  |     ",
        "                      /  /      ",
        "                     / ,'       ",
        "         .-------.--- /         ",
        "        \"._ __.-/ o. o\\         ",   
        "           \"   (    Y  )        ",   
        "                )     /         ",
        "               /     (          ",
        "              /       Y         ",
        "          .-'         |         ",
        "         /  _     \\    \\        ",
        "        /    `. '. ) /' )       ",
        "       Y       )( / /(,/        ",
        "      ,|      /     )           ",
        "     ( |     /     /            ",
        "      \" \\_  (__   (._           ",  
        "          \"-._,)--._,)          ", 
        "                                 ",
        "          RABBIT ROLE            ",
      }




      -- Botões principais
      dashboard.section.buttons.val = {
        dashboard.button("e", "  Novo arquivo", ":ene <BAR> startinsert <CR>"),
        dashboard.button("f", "  Buscar arquivo", ":Telescope find_files<CR>"),
        dashboard.button("r", "  Arquivos recentes", ":Telescope oldfiles<CR>"),
        dashboard.button("c", "  Configuração", ":e ~/.config/nvim/init.lua<CR>"),
        dashboard.button("q", "  Sair", ":qa<CR>"),
      }

      -- Rodapé (opcional)
      dashboard.section.footer.val = "Bem-vindo ao Neovim"

      -- Aplicar configuração
      alpha.setup(dashboard.config)
    end
  }
}

